...
class ModelControl:
    def __init__(self, data, config, lobby_server, irc):
        self.chat_updater = ModelChatUpdater(data, irc.client, config)
...
class Models:
    def __init__(self, config, lobby_server, irc):
        self.data = ModelData()
        self.control = ModelControl(self.data, config["chat"], lobby_server,
                                    irc)
